//
//  ModifyContractViewController.swift
//  Yeah_SalesExpert
//
...
class ModifyContractViewController: UIViewController {
    ...
    @IBAction func bt_add(sender: AnyObject) {
        if tf_name.text == "" || tf_oppo == "" {
            ...
            contract.setId(DataReader.getCurrentContract().getId())
            contract.setName(tf_name.text!)
            ...
            contract.setOppoId(DataReader.getCurrentContract().getOppoId())
            ...
            DataReader.modifyContract(contract)
            
            let alert = UIAlertView.init(title: "修改成功", message: "该合同已经被成功地修改了!", delegate: nil, cancelButtonTitle: "我知道了!")
            alert.show()
    ...
    override func viewWillAppear(animated: Bool) {
        ...
        let currentContract = DataReader.getCurrentContract()
        tf_name.text = currentContract.getName()
        tf_oppo.text = DataReader.getOppotunityWithId(currentContract.getOppoId()).getName()
        stageIndex = currentContract.getContractStage()
        tf_stage.text = String(ContractStage.getContractStageString(stageIndex))
        wayIndex = currentContract.getPayWay()
        tf_way.text = String(PayWay.getPayWayString(wayIndex))
        textView.text = currentContract.getNote()
        
        contract.setOppoId(currentContract.getOppoId())
